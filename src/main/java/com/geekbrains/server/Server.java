...
// отправляем личное сообщение
    public synchronized void privMassage(String nik, String newMassage) {
        for (ClientHandler handler : connectedUsers) {
            if (handler.getNickName().equals(nik)) {

                handler.sendMessage(newMassage);
                return;
            }
 ...
 // список ник в сети
    public synchronized String listNik () {
        String list = "";
        for (ClientHandler handler : connectedUsers) {

                list += handler.getNickName() + "\t";
            }
